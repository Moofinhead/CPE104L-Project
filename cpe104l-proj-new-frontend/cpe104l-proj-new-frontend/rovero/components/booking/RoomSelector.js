...
const RoomSelector = ({ rooms = [], selectedRoom, onSelectRoom, checkInDate, checkOutDate }) => {
  ...
  const roomTypes = [...new Set(rooms
    .filter(room => room?.name)  // only include rooms that have a name
    .map(room => room.name.split(' ')[0]))]; // get unique room types
  ...
  const formatRoomName = (name) => { // format room name into type and number
    if (!name) return { type: '', number: '' }; // handle undefined name
  ...
  return (
    <div className="room-selector">
      ...
      <div className="hotel-floor"> {/* container for displaying rooms */}
        {rooms
          .filter(room => !selectedType || room.name.startsWith(selectedType)) // filter rooms based on selected type
          .map((room) => {
            ...
            return (
              <button
                key={room.id}
                className={`room ${selectedRoom === room.id ? 'selected' : ''} ${available ? 'available' : 'unavailable'}`}
                onClick={() => available && onSelectRoom(room.id)}
                disabled={!available}
              >
